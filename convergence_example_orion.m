function convergence_example_orion()
    solver = 1;
    func = [];
    num_iter = 1000;
    x0_list = linspace(-5,5,num_iter);
    x0_list2 = [];
    filter_params = [1e-15,1e-2,1e-14,1e-2,2];
    plot_Convergence(solver, @test_function01, x0_list, x0_list2, filter_params);
end

function plot_Convergence(solver, fun, x0_list, x0_list2, filter_params)
    ...
    for n=1:length(index_list)
        ...
        if e_list0(n)>filter_params(1) && e_list0(n)<filter_params(2) && e_list1(n)>filter_params(3) && e_list1(n)<filter_params(4) && index_list(n)>filter_params(5)
