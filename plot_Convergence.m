function x = plot_Convergence(solver, func, x0_list, x0_list2)
    global input_list;
    %% Step 4
    num_iter = 1000
    solver = 1
    ...
    x_next_list = [];
    index_list = [];
    deriv = @(x) 3*(x.^2)/100 - 2*x/8 + 2 +(6/2)*cos(x/2+6) - exp(x/6)/6;
    ...
    for n = 1:num_iter
        ...
        %run the solver
        if solver == 1
            x_root = bisectionsolver(func,x0,x1)
        elseif solver == 2
            x_root = newtonsolver({func, deriv},x0);

        elseif solver == 3
            x_root = secantsolver(fun,x0, x1);
